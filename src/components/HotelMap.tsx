...
const HotelMap = () => {
	...
	const mapHandler = async (e: React.MouseEvent<HTMLButtonElement>, el: HotelInterlook) => {
		e.preventDefault();
	...
	function deleteHandler(e: React.MouseEvent<HTMLButtonElement>, el: HotelInterlook) {
		e.preventDefault();
								...
								? hotelProps.map((el, i) => {
									...
									return (
										<div key={el._id}>
											...
											<label htmlFor={`integrationCode-${el._id}`}>integration code: </label>
											<input
												type="text"
