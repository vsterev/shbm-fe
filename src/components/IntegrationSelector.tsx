...
const IntegrationSelector = () => {
    const { integrations, setIntegrations, selectedIntegration, setSelectedIntegration } = useIntegrationContext();

    const token = appCookie('hbs-token');

    useEffect(() => {
        IntegrationService.getIntegrations(token)
    ...
    return (
        <div style={{ margin: '1rem' }}>
            ...
            <select
                ...
                onChange={(e) => {
                    const selectedIntegration = integrations.find((i) => i.name === e.target.value);
