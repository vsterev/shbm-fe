...
import type { AddUser as AddUserType, GetUser } from '../interfaces/user.interface'
...
const Users = () => {
    ...
    const userList = () => userService.allUsers(token)
        .then(usrs => {
            // if (!!usrs.msg) {
            //     logOut();
            // }
    ...
    return (
        <>
            ...
            <div className={style.bookingWrap}>
                ...
                <div onClick={() => toggleAdd()} className={style.toggleButton}>+ add user</div>
                <table className={styles.bookings}>
                    ...
                    <tbody>
                        {Array.isArray(users) && users?.map((el, i) => {
                            return (
                                <React.Fragment key={el._id}>
                                    <tr>
                                        ...
                                        <td>
                                            <button disabled={el.email === 'vasil@solvex.bg'} onClick={() => editHandler(el)} className={style2.submitButton}>edit</button>
                                            <button onClick={() => deleteHandler(el)} disabled={el.email === 'vasil@solvex.bg'} className={style2.deleteButton}>delete</button>
                                        </td>
                                    </tr>
                                </React.Fragment>
                            )
                ...
                {toggleAddUser && <AddUser setUsers={setUsers} toggleAdd={toggleAdd} action="new" />}
                {Object.keys(editUser).length > 0 && <AddUser setUsers={setUsers} setEditUser={setEditUser} editUser={editUser} action="edit" />}
            </div>
        </>
    )
}
