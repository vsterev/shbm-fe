...
const GetBookings = () => {
	...
	useEffect(() => {
		...
		BookingService
			.get({ status: 'new', next: false }, token, selectedIntegration.name)
			.then((rs) => {
				// if (rs.msg) {
				//   logOut();
				// }
		...
		BookingService
			.get({ status: 'change', next: false }, token, selectedIntegration.name)
			.then((rs) => {
				if (Array.isArray(rs) && rs.length === 0) {
		...
		BookingService
			.get({ status: 'cancel', next: false }, token, selectedIntegration.name)
			.then((rs) => {
				if (Array.isArray(rs) && rs.length === 0) {
	...
	const submitHandler = async (data: { type: 'new' | 'change' | 'cancel' }) => {
		...
		await BookingService.send(sendArr, token, selectedIntegration.name).then((rs) => {
