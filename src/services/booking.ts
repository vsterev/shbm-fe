...
const BookingService = {
	...
	get: async (
		...
		try {
			const sendParams = new URLSearchParams(`status=${data.status}&integrationName=${integrationName}&next=${data.next}`);
	...
	send: async (
		...
		try {
			const response = await fetch(`${API_URL}/bookings?integrationName=${integrationName}`, {
